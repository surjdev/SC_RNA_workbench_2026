...
from sc_workbench.io import from_dataframe, from_numpy, to_dataframe, to_numpy
...
def test_pandas_numpy_slicing_and_metadata(sample_pandas_df):
    ...
    batch_series = pd.Series(["batch_1" if i % 2 == 0 else "batch_2" for i in range(wb.n_cells)], index=wb.obs.index)
