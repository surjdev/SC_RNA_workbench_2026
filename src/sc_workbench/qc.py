...
def calculate_qc_metrics(
    ...
    adata.var["mt"] = [
        any(str(g).startswith(p) for p in mito_prefix) for g in adata.var.get("gene_name", adata.var_names)
    ]
    # Tag ribosomal protein genes
    adata.var["ribo"] = [
        any(str(g).startswith(p) for p in ribo_prefix) for g in adata.var.get("gene_name", adata.var_names)
    ]
...
def detect_doublets(
    ...
    try:
        ...
        console.print(f"[bold green]✔ Doublets detected:[/bold green] {n_doublets} / {adata.n_obs} cells ({n_doublets/adata.n_obs*100:.1f}%)")
...
def filter_genes(adata: ad.AnnData, min_cells: int = 3) -> ad.AnnData:
    ...
    console.print(f"[bold green]✔ Filtered Genes:[/bold green] Retained {adata.n_vars} / {initial_genes} genes (min_cells >= {min_cells})")
    return adata
