...
class SingleCellWorkbench:
    ...
    @classmethod
    def from_matrix(
        cls, file_path: Union[str, Path], transpose: bool = True, sparse: bool = True
    ) -> "SingleCellWorkbench":
    ...
    def from_df(
        ...
        adata = io.from_dataframe(
            df, obs=obs, var=var, is_genes_by_cells=is_genes_by_cells, sparse=sparse
        )
        return cls(adata)
    ...
    def from_numpy(
        ...
        adata = io.from_numpy(
            X, cell_names=cell_names, gene_names=gene_names, obs=obs, var=var, sparse=sparse
        )
        return cls(adata)
    ...
    def add_obs(
        self, data: Union[pd.Series, pd.DataFrame, np.ndarray, list], col_name: Optional[str] = None
    ) -> "SingleCellWorkbench":
        ...
        if isinstance(data, pd.DataFrame):
            ...
            self.adata.obs[col_name] = (
                io._align_metadata(data.to_frame(name=col_name), self.adata.obs_names)[col_name]
                if isinstance(data, pd.Series)
                else data
            )
        else:
            raise ValueError(
                "col_name must be specified when adding a Series, list, or NumPy array."
            )
        return self
    ...
    def get_embedding(
        self, name: str = "X_umap", as_df: bool = True
    ...
    def calculate_qc(
        ...
        self.adata = qc.calculate_qc_metrics(
            self.adata, mito_prefix=mito_prefix, ribo_prefix=ribo_prefix
        )
        return self
    ...
    def normalize(self, target_sum: float = 1e4, save_raw: bool = True) -> "SingleCellWorkbench":
        ...
        self.adata = preprocess.normalize_and_log(
            self.adata, target_sum=target_sum, save_raw=save_raw
        )
        return self

    def select_hvg(
        self, n_top_genes: int = 2000, flavor: str = "seurat", batch_key: Optional[str] = None
    ) -> "SingleCellWorkbench":
        """Select Highly Variable Genes."""
        self.adata = preprocess.select_hvg(
            self.adata, n_top_genes=n_top_genes, flavor=flavor, batch_key=batch_key
        )
        return self
    ...
    def compute_neighbors(
        self, n_neighbors: int = 15, n_pcs: int = 30, use_rep: Optional[str] = None
    ) -> "SingleCellWorkbench":
        """Build cell-cell k-NN graph."""
        self.adata = reduction.compute_neighbors(
            self.adata, n_neighbors=n_neighbors, n_pcs=n_pcs, use_rep=use_rep
        )
        return self
    ...
    def cluster(
        ...
        if estimator is not None:
            ...
            self.adata = clustering.cluster_louvain(
                self.adata, resolution=resolution, key_added=key
            )
    ...
    def apply_sklearn(
        ...
        if res.ndim == 1:
            ...
            if (
                layer is not None or input_source == "X" or input_source in self.adata.layers
            ) and res.shape[1] == self.n_genes:
                dest = key_added or est_name.lower()
    ...
    def train_classifier(
        self, classifier: Any, target_col: str = "leiden", use_rep: str = "X_pca", cv: int = 5
    ) -> Dict[str, Any]:
        """
    ...
    def find_markers(
        ...
        self.adata = markers.find_markers(
            self.adata, groupby=groupby, method=method, n_genes=n_genes
        )
        return markers.get_markers_df(
            self.adata, pval_cutoff=pval_cutoff, logfc_cutoff=logfc_cutoff
        )
    ...
    def score_signatures(
        self, signatures: Dict[str, List[str]], prefix: str = "sig_"
    ) -> "SingleCellWorkbench":
    ...
    def annotate_cell_types(
        ...
        self.adata = annotation.assign_cell_types(
            self.adata, cluster_to_celltype, cluster_key=cluster_key, new_key=new_key
        )
        return self
    ...
    def infer_trajectory(
        self, groups: str = "leiden", root_cluster: Optional[Union[str, int]] = None
    ) -> "SingleCellWorkbench":
