...
def run_pca(
    ...
    mask_var = "highly_variable" if ("highly_variable" in adata.var and adata.var["highly_variable"].sum() > 0) else None
    sc.pp.pca(
        adata,
...
def run_harmony(
    ...
    console.print("[bold green]✔ Harmony integration complete (stored in .obsm['X_pca_harmony']).[/bold green]")
    return adata
...
def compute_neighbors(
    ...
    console.print(f"[bold cyan]Building k-NN graph ({n_neighbors} neighbors, {n_components} components from {use_rep})...[/bold cyan]")
...
def run_umap(
    ...
    console.print("[bold green]✔ UMAP coordinates computed (stored in .obsm['X_umap']).[/bold green]")
    return adata
...
def run_tsne(
    ...
    console.print("[bold green]✔ t-SNE coordinates computed (stored in .obsm['X_tsne']).[/bold green]")
    return adata
