...
def run_paga(
    adata: ad.AnnData, groups: str = "leiden", use_rna_velocity: bool = False
) -> ad.AnnData:
    """
    ...
    console.print(
        "[bold green]✔ PAGA connectivity matrix computed (stored in .uns['paga']).[/bold green]"
    )
    return adata
...
def run_dpt(
    ...
    """
    console.print(
        "[bold cyan]Computing Diffusion Maps and Diffusion Pseudotime (DPT)...[/bold cyan]"
    )
    ...
    console.print(
        "[bold green]✔ Diffusion pseudotime computed (stored in .obs['dpt_pseudotime']).[/bold green]"
    )
    return adata
