...
def score_gene_signatures(
    ...
    """
    console.print(f"[bold cyan]Scoring {len(signatures)} gene signatures across cells...[/bold cyan]")
    
    for sig_name, genes in signatures.items():
        # Keep genes present in adata
        valid_genes = [g for g in genes if g in adata.var_names]
        if len(valid_genes) == 0:
            console.print(f"[yellow]⚠ No valid genes found in dataset for signature '{sig_name}'![/yellow]")
            continue
        ...
        console.print(f"[bold green]✔ Scored '{sig_name}':[/bold green] {len(valid_genes)} genes -> .obs['{score_key}']")
...
def assign_cell_types(
    ...
    console.print(f"[bold green]✔ Cell types assigned successfully:[/bold green]")
    for ct, count in cell_types.items():
        console.print(f"  • [magenta]{ct}[/magenta]: {count} cells ({count/adata.n_obs*100:.1f}%)")
